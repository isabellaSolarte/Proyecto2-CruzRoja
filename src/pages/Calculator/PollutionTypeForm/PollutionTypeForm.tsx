...
const PollutionTypeForm = ({ nextStep, stepBack }: PollutionTypeFormProps) => {
  ...
  return (
    <ManagmentLayout
      ...
      actionsContent={
        <CustomIconDetails
          description={t('calculator.pollutionTypeForm.helpTypes')}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Box mt={5} sx={{ borderTop: '1px solid #C8C8C8' }}>
                  ...
                  <>
                    {categoria.pollutans.map(pollutionType => (
                      <PollutionTypeCard
                        ...
                        positiveAction={() => {
                          updatePollutionType(pollutionType.name, true);
                        }}
                        negativeAction={() => {
                          updatePollutionType(pollutionType.name, false);
                        }}
            ...
            {!!errors.pollutionType && (
              <Grid item xs={12}>
                <ErrorText error={errors.pollutionType.message} formErrorKey={'calculator'} />
              </Grid>
            )}
          </Box>
          <Box
