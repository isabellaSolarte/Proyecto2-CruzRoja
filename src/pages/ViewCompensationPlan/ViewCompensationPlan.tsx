      ...
      buttonDetails: [
        {
          ...
          color: 'warning',
          icon: <VisibilityIcon />,
          onClick: (rowData: { id: number }) => {
            handleViewButtonClick(rowData.id);
          },
        },
      ],
    }),
    ...
    <ManagmentLayout
      ...
      generalContents={
        <Grid>
          <Box mb={10}>
            <CustomText
              texto={currentPlan.description}
              variante="subtitulo"
              styles={{ fontWeight: 'normal', marginTop: '16px', marginBottom: '20px' }}
            />
            <div style={{ display: 'inline-block' }}>
              <h3 style={{ fontWeight: 'bold', display: 'inline' }}>Compensación total:</h3>
              <h3 style={{ fontWeight: 'normal', display: 'inline' }}>
                {' '}
                {currentPlan.ufpCompensation} ufp
              </h3>
            </div>
            <br />
            <div style={{ display: 'inline-block' }}>
          ...
          {showModal && (
            <CustomModal
              open={showModal}
              title={<CustomText texto={actionSelect?.name ?? ''} variante="titulo" />}
              description={
              ...
              generalContents={
                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    />
                    <CustomText
                      texto={` ${actionSelect?.footPrintUnity ?? ''} `}
                      variante="texto"
                    />
                  </Box>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    />
                    <CustomText
                      texto={` ${actionSelect?.unitaryPrice ?? ''} COP`}
                      variante="texto"
                    />
                  </Box>
                </Box>
          ...
          {showModalAcquire && (
            <CustomModal
              ...
              title={<h2>Adquirir plan de compensación</h2>}
              description={
                <Box>
                  <div style={{ display: 'inline-block' }}>
